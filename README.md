...
### 管理监控面板
- **Web 面板** — 挂机启动后自动开启 HTTP 面板，支持手机与电脑访问
- **实时监控** — 账号状态、农场概览、好友巡查、操作限制、数据趋势
- **最近日志** — 实时查看运行日志，数据每 3 秒自动刷新
...
挂机启动后，管理面板自动运行在 **http://localhost:3000**，可：

- 电脑浏览器访问 `http://localhost:3000`
- 手机浏览器访问 `http://<电脑IP>:3000`（需在同一局域网）

面板功能：账号状态、农场概览、好友巡查、每日限制、操作统计、数据趋势图、最近日志。  
修改端口：编辑 `src/config.js` 中的 `adminPort`。

#### 面板登录

面板已加入登录保护，默认密码为 `admin`。可通过环境变量 `ADMIN_PASSWORD` 修改：
